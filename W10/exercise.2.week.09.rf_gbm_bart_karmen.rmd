...
df  <- read_rds("W10/karmen_data_with_sex_and_age.rds") 
...
now do it with caret
```{r}

set.seed(0)

trainIndex <- createDataPartition(age_data$age, p = .8, 
                                  list = FALSE, 
                                  times = 1)

age_train <- age_data[ trainIndex,]
age_test  <- age_data[-trainIndex,]
...
rf.age = randomForest(x = xtrain, y=ytrain, keep.forest = T,
...
rf.age = train(xtrain, ytrain, method = "rf", ntree = 500, trControl = fitControl, tuneLength = 2)
rf.age
...
```{r}
yhat.rf.age=predict(rf.age, xtest)
mean((yhat.rf.age - ytest)^2)
```

```{r}
plot(x=seq(1,rf.age$ntree,by=1),rf.age$mse)
```
---
...
If you have too many trees you will overfit the data.
but i think 300 is enough
...
it looks like we have 7 important molecules
...
num_predictors <- ncol(xtrain)

# Train a random forest using all predictors (bagging)
rf_all_predictors <- randomForest(x = xtrain, y = ytrain, keep.forest = TRUE,
								  xtest = xtest, ytest = ytest,
								  ntree = 500, importance = TRUE,
								  do.trace = TRUE, mtry = num_predictors)

# Train a random forest using 1/10 of predictors for each split
num_predictors_to_use <- num_predictors / 10
rf_fraction_predictors <- randomForest(x = xtrain, y = ytrain, keep.forest = TRUE,
									   xtest = xtest, ytest = ytest,
									   ntree = 500, importance = TRUE,
									   do.trace = TRUE, mtry = num_predictors_to_use)

# Compare the mean squared error of the two models
mse_all_predictors <- mean((predict(rf_all_predictors, xtest) - ytest)^2)
mse_fraction_predictors <- mean((predict(rf_fraction_predictors, xtest) - ytest)^2)

mse_all_predictors
mse_fraction_predictors
...
```{r}
boost.age = gbm(age ~ ., data = age_train, distribution = "gaussian", n.trees = 1000, interaction.depth = 1, shrinkage = 0.1, cv.folds = 5)
```

```{r}
caret.boost.age = train(age ~ ., data = age_train, method = "gbm", trControl = fitControl, verbose = FALSE)
caret.boost.age
```
...
```{r}
plot(x=seq(1,boost.age$n.trees,by=1),boost.age$cv.error)
```
...
cv error stabilizes around 200 trees 
...
```{r}
boost.age_deep = gbm(age ~ ., data = age_train, distribution = "gaussian", n.trees = 1000, interaction.depth = 3, shrinkage = 0.1, cv.folds = 5)

```
```{r}
plot(x=seq(1,boost.age_deep$n.trees,by=1),boost.age_deep$cv.error)
```

much gooder 5 is the perfect amount because we didnt look at others
---
...
bart.mse=mean(residuals_bart^2)
...
caret.bartfit.age =  train(xtrain, ytrain, method = "bartMachine", trControl = fitControl, tuneLength = 2)
caret.bartfit.age
```
...
rf.mse = mean((predict(rf.age, newdata=xtest) - ytest)^2)
#boost.mse = mean((predict(boost.age, newdata=xtest, n.trees=1000) - ytest)^2)
bart.mse = bart.mse
```
...
```{r}
rf.mse = mean((predict(rf.age, newdata=xtest) - ytest)^2)
boost.mse = mean((predict(caret.boost.age, newdata=xtest) - ytest)^2)
bart.mse = bart.mse
```
...
```{r}
lgbm.age = lgb.train(data = lgb.Dataset(data = as.matrix(xtrain), label = ytrain), nrounds = 10000)
```

```{r}
lgbm.age_pred = predict(lgbm.age, as.matrix(xtest))
```
```{r}
lgbm.mse = mean((lgbm.age_pred - ytest)^2)
lgbm.mse
```
