...
trainfold <- data_train %>% sample_frac(size=0.95)
...
cv.fit <- cv.glmnet(x_train, y_train, alpha = 1, nfolds = 20, relax=TRUE)
...
# Make predictions on the prediction data
predicted <- glmnet:::predict.elnet(final.fit, newx = x_predict, s = cv.fit$lambda.min, type = "response")
...
files   <- Sys.glob("als_progression.The_*.rds")
